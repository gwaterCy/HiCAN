...
class HiCAN(Model):
    def __init__(self,decay,penalization=True,lambda_lossL2=1e-3,windowSize=3,batch_size=512
                 ,emb_dim=100,num_item=41372,max_length=40,learning_rate=0.001,learningRate_decay=0.1):
        super(HiCAN, self).__init__()
    ...
    def _build(self):
        ...
        self.layers.append(LocalCAN(hidden_dim=self.emb_dim,
                                           mask=self.mask,
                                           dropout=self.dropouts[0],
                                           windows_size=self.windowSize,
                                           activation="sigmoid"))
        self.layers.append(GlobalCAN(max_length=self.max_length,
